...
const setCell = (x: number, y: number, c: string) => {
  currentPuzzle[x][y] = c;
};

const setCellManual = (x: number, y: number, c: string) => {
...
const getPuzzleRowCells = (y: number) => {
  const result: string[] = [];
  for (let x = 0; x < 9; x++) {
...
const getPuzzleColCells = (x: number) => {
  const result: string[] = [];
  for (let y = 0; y < 9; y++) {
    result.push(currentPuzzle[x][y]);
  }
...
const getPuzzleBoxCells = (b: number) => {
  const result: string[] = [];
  for (let x = 0; x < 9; x++) {
    for (let y = 0; y < 9; y++) {
      if (getBox(x, y) === b) {
...
const getPuzzleBoxCellXYs = (b: number) => {
  const result: number[][] = [];
  for (let x = 0; x < 9; x++) {
    for (let y = 0; y < 9; y++) {
      if (getBox(x, y) === b) {
...
const sudokuTable = document.getElementById("sudoku-table") as HTMLTableElement;
for (let x = 0; x < 9; x++) {
...
const numbers = document.getElementById("numbers") as HTMLDivElement;
for (let b = 1; b < 10; b++) {
...
const getCellInfo = (x: number, y: number) => {
  return {
    ...
    box: Array.from(document.querySelectorAll(`[data-box="${getBox(x, y)}"]`))
      ...
      .filter(Boolean),
    num: Number(
      document.querySelector(`[data-y="${y}"][data-x="${x}"]`)?.textContent
    ),
  };
};
...
const getPossibleSolutions = (x: number, y: number) => {
  const info = getCellInfo(x, y);
  if (info.num) {
    return new Set([]);
  }
  return numbersSet.difference(
    new Set(info.row).union(new Set(info.col)).union(new Set(info.box))
  );
};
...
let temp: number[][] = [];
let oldPuzzle: string[][] | null = null;
...
const solveBoard = async () => {
  ...
  for (let x = 0; x < 9; x++) {
    for (let y = 0; y < 9; y++) {
      for (let test = 1; test < 10; test++) {
        ...
        const testBoxCells = getBoxCells(getBox(x, y)).filter(
          ([boxX, boxY]) => {
            return Array.from(getPossibleSolutions(boxX, boxY)).includes(
              `${test}`
            );
          }
        );
        ...
        const testRowCells = getRowCells(y).filter(([rowX, rowY]) => {
          return Array.from(getPossibleSolutions(rowX, rowY)).includes(
            `${test}`
          );
        });
        ...
        const testColCells = getColCells(x).filter(([colX, colY]) => {
          return Array.from(getPossibleSolutions(colX, colY)).includes(
            `${test}`
          );
        });
  ...
  if (!isBoardSolved()) {
    ...
    currentPuzzle = oldPuzzle as string[][];
    // oldState = "";
    oldPuzzle = null;
    temp = [];
    return solveBoard();
...
const solveBoardButton = document.getElementById(
  "solve-board"
) as HTMLButtonElement;
solveBoardButton.addEventListener("click", solveBoard);
const clearBoardButton = document.getElementById(
  "clear-board"
) as HTMLButtonElement;
clearBoardButton.addEventListener("click", clearBoard);

const randomizeBoardButton = document.getElementById(
  "randomize-board"
) as HTMLButtonElement;
randomizeBoardButton.addEventListener("click", randomizeBoard);

const logBoardButton = document.getElementById(
  "log-board"
) as HTMLButtonElement;
